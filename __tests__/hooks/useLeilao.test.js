...
describe('hooks/useLeilao', () => {
  ...
  it('deve retrbar uma função para autializar como segundo parâmetro', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useLeilao());
    await waitForNextUpdate();

    //armazenando os dados que quero que seja enviado no setState
    obtemLeilao.mockReturnValue(mockLeilaoAtualizado);
    obtemLancesDoLeilao.mockReturnValue(mockLancesDoLeilaoAtualidado);

    //ordena que o setState dispare de fato para que haja a atualização do estado
  ...
  describe('[_, _, enviaLance]', () => {
    ...
    it('deve rejeitar o lance caso o valor não for numério', async () => {
      const { result, waitForNextUpdate } = renderHook(() => useLeilao());
      await waitForNextUpdate();
